...
help = '''  (all)
                 docsum             xml      DocumentSummarySet XML
                 docsum             json     DocumentSummarySet JSON
                 full               text     Full Document
                 uid                xml      Unique Identifier List
                 uid                text     Unique Identifier List
                 url                text     Entrez URL
...
for line in help:
    ...
    # Format lines have 2+
        ...
        if parent_format not in db[section]:
            db[section][parent_format] = (parent_format, description)
    elif len(line) == 3:
        parent_format = line[0]
        ...
        if parent_format not in db[section]:
            db[section][parent_format] = (parent_format + '-' + format_modifier, description)
